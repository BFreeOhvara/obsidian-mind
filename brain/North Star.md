...
**Scoped 2026-07-26 (was TBD since 2026-07-23):** in-system messaging is now part of a broader Team page rebuild — see Prompt 357 in LIVE_STATE. Hierarchy is being renamed "Team" with 3 sub-tabs (Hierarchy/Messages/Meetings): real chat with notifications, plus a Meetings tab for team scheduling with Zoom (embedded if iframe-able, new-tab link if not — investigate first, don't assume, same lesson learned from Carrier Portals blocking iframes).
...
**Future product idea, brainstormed 2026-07-25 — deliberately NOT current priority.** Brayden's own framing: "the goal right now is to utilize the system to grow our own business" — get Ohvara itself running well first, then eventually sell the whole system (dashboard + a client-facing retention app Jordan is building, "Ensure Suite") to other agency owners as a licensed product. Closing real agency customers isn't expected soon except maybe word-of-mouth; ads/active selling of this come later, once Ohvara is a proven, lived-in user of its own system. Key shape of the idea, captured so it isn't lost, none of it built or queued:
- This would require true multi-tenancy — each purchasing agency as an isolated org (own hierarchy tree, own data), not another branch on Ohvara's own single hierarchy tree the way invites work today.
- Only orgs with an active paid subscription can invite new agents (billing-status gate).
- New-agent invites require a vetting step reviewed by Ohvara (not the agency owner): applicant submits info via request link — legal name, NPN, state(s)/license number(s)/lines of authority, license expiration, proof of active E&O coverage — Ohvara approves or rejects before they join. NPN+state gets a free lookup on NIPR's public PDB or the state DOI site; no paid verification API needed at first.
- Pricing direction (ranges, not final numbers): $2,500 setup + $2,500 first-month retainer (pitched as "$5k to start"), then $2,500–$4,000/mo covering the first 5–7 agents (exact included-count still undecided), $100–$250 per additional seat/month beyond that. Lead/ad spend is always separate, never bundled into these numbers — agents cost ~$4,620–$7,700/mo each in lead spend alone (6–10 leads/day at $35/lead, 22 working days/mo), whether the owner pays for all agents or each agent pays their own.
- Revisit and firm up specifics (exact included-agent count, exact per-seat price, who reviews vetting requests) when this actually becomes near-term, not before.

**Clarified 2026-07-26 — no separate admin dashboard needed for Ohvara's own team, and the NPN/vetting/24-hour-approval flow above is future-licensing-only, not needed now.** Brayden reconsidered from scratch: for Ohvara's own internal team (himself + Nate, Jordan, Rego), one shared dashboard is enough — the only difference for his own `admin` role vs everyone else is **the ability to send invite links; nothing else**. No vetting, no NPN collection, no 24-hour wait, no approve/deny queue — that whole flow belongs to the future licensed-agency product above, for when Ohvara is selling seats to other agency owners' teams (where Ohvara itself needs to vet who a stranger agency owner is inviting). For his own known team, he just invites Nate/Jordan/Rego directly, they're in, done. A real admin dashboard (for tracking sold licenses/customers) is explicitly a later idea once selling starts, not something to build now.

**Concrete near-term change SHIPPED 2026-07-26, fully closed:** invite-link generation on Hierarchy moved from the closer view to the admin-only view (UI, `4e97c12`), and the backend RLS on `rep_invites_insert` was tightened to match (`created_by = auth.uid() and is_admin()`, `or role = 'closer'` removed) — applied directly by Falcon via Supabase MCP with Brayden's go-ahead after CC's own auto-mode classifier blocked it. Confirmed live via `pg_policies` post-migration. No open decision left here.

**Prompt 357 (Team page rebuild) fully SHIPPED 2026-07-26 — frontend + backend both done.** `ohvara-dashboard` `0930794`: Hierarchy renamed to Team with 3 sub-tabs (Hierarchy unchanged, Messages built for real, Meetings a coming-soon placeholder — the video-room idea deferred, not built). Backend migration `084_team_chat.sql` (new `team_conversations`/`team_messages` tables, RLS, notify trigger, realtime) was blocked by CC's auto-mode classifier same as the invite-RLS change above — CC routed the exact SQL back through LIVE_STATE (Prompt 362), Falcon applied it directly via Supabase MCP with Brayden's go-ahead, confirmed live via `list_tables`/`pg_policies`. Team chat + DMs work immediately, no further action needed anywhere.

**Prompt 384 (real notification triggers) SHIPPED frontend/backend-code 2026-07-29, BLOCKED on two live-DB actions needing Brayden's go-ahead.** `ohvara-dashboard` `fa8c38f` — new migration `090_real_notification_triggers.sql` adds real SECURITY DEFINER triggers for 4 of the 9 confirmed types (new bug report, new team member, lapse check-in due, leaderboard #1 + 50%/100% goal milestones — team/direct message were already real). Same pattern as 084/362 above: CC's own DB-write classifier won't let it run `apply_migration` or a live `DELETE`, so both need Falcon (or CC with an explicit go-ahead in chat) to run them directly via Supabase MCP:
1. **Run migration `090_real_notification_triggers.sql`** (already committed to `ohvara-dashboard`, verified schema-accurate against the live DB read-only first) in the Supabase SQL editor for project `jjextitmbptoaolacocs`.
2. **Then clear brayden11's notifications** (all 17 Prompt 382 preview rows + the original stray one) — the prompt's own Step 1, deliberately not run automatically since it's a live delete on Brayden's real data: `delete from notifications where profile_id = '5164aa43-e803-41c5-b96f-1663a53c7404';`

**Not built: "missed call from a lead" (item 1 of the 9)** — genuinely unbuilt infrastructure, not a missing notification insert. `twilio-voice-webhook` only handles the agent's own outbound browser-to-lead dial; there is no inbound call-routing/AI-receptionist code anywhere in this repo to hook a trigger into (matches Phase 1's "AI receptionist... net new" line above). Needs a design call with Brayden on where that system will actually live before this can be scoped, let alone built. Full write-up: [[Memories]] Prompt 384 entry.
