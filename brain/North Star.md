...
**Concrete near-term change SHIPPED 2026-07-26 (UI only, `4e97c12`):** invite-link generation on Hierarchy moved from the closer view to the admin-only view — closers no longer see "Grow your downline" at all, only admin does. **Backend RLS is NOT yet tightened to match** — `rep_invites_insert` (migration 072) still lets any authenticated agent insert their own 'closer' invite (`created_by = auth.uid() and (is_admin() or role = 'closer')`), confirmed live via `execute_sql` against `pg_policies`. A migration to restrict that INSERT to `is_admin()` only was written but the auto-mode classifier blocked applying it (production RLS change needs explicit sign-off, not something CC applies unprompted). **Open decision for Brayden:** approve the migration (drop+recreate `rep_invites_insert` with `created_by = auth.uid() and public.is_admin()`) so the backend actually matches the UI, or confirm the UI-only gate is enough for now since it's an internal small team. Related: [[Gotchas]] (harness pattern), `ohvara-dashboard` commit `4e97c12`.
