...
### 🆕 Prompt 390 QUEUED 2026-07-29 — Double the size of the floating bug-report button

The floating "Report a bug" button ([[Prompt 381]], bottom-right corner, every page) is too small per Brayden — double its current size (both the button itself and the bug icon inside it, keep it visually proportional, don't just scale the icon while leaving a tiny hit-target ring around it). Keep the same bottom-right placement/spacing from the screen edge, same role-conditional behavior (submit form for agents, inbox for admin) — this is a pure sizing tweak, no behavior change.

Verify with a screenshot showing the button at its new, larger size.

---

### 🆕 Prompt 389 QUEUED 2026-07-29 — Replace "Approval Rate" stat card with "Early Cancellation Rate"

Brayden's call after thinking through how underwriting actually works at Ohvara: ~90% of policies get an instant approve/decline decision at the point of sale, not a delayed underwriting review — and there's no reason to log an instant decline into the dashboard at all (no client, no commission, not really a "submission"). That means the only cases that ever land in `Not Approved` are the rare ones that genuinely went to underwriting first — too small a sample to make "Approval Rate" a meaningful metric on your real book. **Remove Approval Rate, replace it in the same card slot with Early Cancellation Rate** — Performance → Production tab's bottom stat row, both You/Team scopes, all 3 date-range toggles.

**Why this is a better fit for the slot:** unlike approval status, cancellations are already reliably tracked — `cancellation_status` (`Cancellation Pending`/`Cancellation Complete`), `cancellation_call_at`, and the existing Cancellation Calendar tab (Submissions page) are a whole workflow built specifically around this, so the data doesn't depend on an agent remembering to update a rarely-touched field. A policy that cancels almost immediately after issue is a real quality-of-sale signal (bad fit/buyer's remorse), unlike the near-empty `Not Approved` bucket.

**Formula (flat 30-day window — flag this explicitly to Brayden as an approximation, not per-state accuracy):**

`Early Cancellation Rate = count(policies where cancellation_status is not null AND cancellation_call_at <= effective_date + interval '30 days') / count(policies where status = 'In Effect' OR cancellation_status is not null)`

Real free-look periods vary by state (typically 10–30 days) and there's no per-state table in this schema to look that up precisely — using a flat 30-day window is a reasonable approximation, but call this out plainly in the close-out note so Brayden knows it's not state-exact, in case he wants a real per-state table built later.

Leave the underlying data model alone otherwise — `Not Approved`/`Undrafted`/the Needs Follow-up tab all stay exactly as built ([[Prompt 369]]), this is purely a stat-card swap.

Verify with a screenshot of the Performance page's bottom stat row showing "Early Cancellation Rate" in Approval Rate's old slot, rendering a real percentage (or the same placeholder pattern as its neighbors if no qualifying data yet).
