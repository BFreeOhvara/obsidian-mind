...
*(Prompts 1, 2, 5–17, 26, 28–181 shipped — Prompt 42 superseded by 44 Fix 2, Prompt 108 superseded by 109, Prompt 110 superseded by 111, Prompt 113 superseded by 114, Prompt 324 shipped 2026-07-21, Prompt 360 shipped 2026-07-26, Prompt 361 shipped 2026-07-26, Prompt 359 shipped 2026-07-26, Prompt 358 shipped 2026-07-26, Prompt 357 fully closed 2026-07-26 (frontend + migration 084), Prompt 356 shipped 2026-07-26, Prompt 362 closed 2026-07-26, Prompt 365 closed 2026-07-26, Prompt 364 closed 2026-07-26, Prompt 363 closed 2026-07-26, Prompt 366 closed 2026-07-26, Prompt 367 closed 2026-07-26, Prompt 368 shipped 2026-07-26, Prompt 369 shipped 2026-07-26 (migration 085), Prompt 371 closed 2026-07-27, Prompt 375 closed 2026-07-27, Prompt 374 closed 2026-07-27, Prompt 373 closed 2026-07-27, Prompt 370 closed 2026-07-27, Prompt 372 closed 2026-07-27, Prompt 376 closed 2026-07-27, Prompt 377 closed 2026-07-27 (real bug was a PostgREST 1000-row cap silently truncating the grid, not MOO/NLG abbreviations — the carrier names were already correct in the DB), Prompt 379 closed 2026-07-28 (URGENT admin data-leak fix, done first out of file order), Prompt 378 closed 2026-07-28 (Carrier/Product dropdowns), Prompt 380 closed 2026-07-28 (form copy/field cleanup), Prompt 381 closed 2026-07-28 (bug-report button), Prompt 383 closed 2026-07-29 (notification bell per-type icons), Prompt 386 closed 2026-07-29 (Performance toggle/date-nav layout shift), Prompt 385 closed 2026-07-29 (First Premium Applied Rate card) — see [[Memories]] for the full trail.)*

### 🆕 Prompt 387 QUEUED 2026-07-29 — New Submission form: State dropdown opens upward instead of downward

Screenshot shows clicking "Select a state" on the New Submission form (Submissions → New Submission) opens the option list upward, covering the whole page above the field, instead of dropping down below the trigger like a normal dropdown. There's plenty of room below it on the page, so this isn't a legitimate viewport-edge auto-flip — it's just opening the wrong direction. Fix the State `SelectField` (and check the new Carrier/Product dropdowns from [[Prompt 378]], since they share the same component pattern — confirm they don't have the same issue) to always anchor/open downward from the trigger, not upward, under normal circumstances.

Verify with a screenshot of the State dropdown open, showing the list appearing below the "Select a state" field.

---

### 🟦 Prompt 384 QUEUED 2026-07-29 — Notifications: final 9 types confirmed, build the REAL triggers (supersedes Prompt 382's preview-catalog approach)

Brayden reviewed the [[Prompt 382]] preview list here in chat rather than logging into the live dashboard — that's the completed review, no further UI keep/cut pass needed. Verdict: **build these 9 as real, live-triggered notifications.** Stop treating this as a preview catalog — these are the final spec.

**Step 1 — clear the account.** Delete every row currently in `notifications` for `profile_id = 5164aa43-e803-41c5-b96f-1663a53c7404` (Brayden's `brayden11` account) — that's all 17 [[Prompt 382]] preview rows plus the original stray one. He does not want fake/sample rows sitting in his real notification box once we know what the real 9 are.

**Step 2 — build real triggers for exactly these 9, nothing else:**

1. **Missed call from a lead** — trigger: a call comes in and goes unanswered by the assigned/queued agent (Retell AI / Twilio call-handling stack). Notify the agent it was routed to. Brayden confirmed this stays as-is even though the full AI-receptionist call-queue routing (multiple agents cycling on no-answer) isn't necessarily built yet — implement against whatever the current call-routing logic actually is, don't block on the receptionist-queue feature.
2. **Lapse check-in due** — trigger: [[Prompt 369]]'s existing monthly lapse-check-in banner logic (anchored to the policy's effective-date day-of-month) — when that banner fires for a policy, also fire a bell notification for the owning agent at the same time. Reuse the existing scheduled mechanism, don't build a second one.
3. **Team chat message** — already implemented (channel messages, [[Prompt 379]] fixed the admin over-broad-read bug on this). No new work, just keep as one of the final 9.
4. **Direct message** — already implemented, same as above, keep.
5. **New team member joined** (admin-only) — trigger: a new `profiles` row is created via invite acceptance. Notify admin(s) only.
6. **New bug report submitted** (admin-only) — trigger: a new row lands in `bug_reports` ([[Prompt 381]]). This should just be wiring an existing INSERT to also create a notification row — the bug-report feature itself is already built, this is the missing notify-on-insert piece.
7. **Moved up to #1 on the leaderboard** — trigger: whatever process computes monthly leaderboard rank (see Leaderboard feature, [[Prompt 374]]) detects an agent's rank changed and their new rank is #1. Needs a "previous rank" comparison, not just current standing — don't fire this every time the leaderboard is simply viewed/recalculated with no actual change.
8. **Hit 50% of monthly goal** — trigger: agent's submitted AP this month crosses 50% of their monthly goal (set on the Profile page, same value driving the Overview "Monthly goal" card). Fire once per agent per month when the threshold is crossed, not on every submission after.
9. **Hit 100% of monthly goal** — same mechanism as #8, threshold 100%, separate one-time fire.

**Explicitly cut, do not build:** activity reminder, commission posted, policy submitted, policy approved, policy sent to underwriting, not approved (Brayden's own reasoning: the dashboard can't know a policy's real-world approval status before the agent tells it by clicking a button — so notifying them about their own click is meaningless), new lead assigned, contracting submission approved, new carrier added. If any of these ideas come back later it'll be a fresh ask, not a revival of this list.

Verify with a screenshot of Brayden's real notification bell showing it starts empty (post-clear) and note in the close-out entry which of the 9 triggers were fully wired vs which need a follow-up (e.g. leaderboard rank-change detection and goal-threshold detection are the least trivial — flag honestly if either needs a design call before shipping rather than half-implementing).
