...
### 🆕 Prompt 370 QUEUED 2026-07-26 — Real Compensation Grid page, seeded from Brayden's actual Eterna contract data

Real, non-speculative build — Brayden pulled this data himself from Eterna's Compensation Grid page (the IMO platform his carrier contracts run through) since it's not published anywhere on the open web; confirmed with him this is the carrier's own commission schedule, gated behind agent/IMO portal access rather than some Eterna-specific secret. Reference data lives at `brain/commission-schedule-tier70.csv` in this vault — **the filename is misleading (kept from an earlier draft), it now holds the FULL 50–145 tier grid, not just the tier-70 column** — read the whole file, don't assume it's narrower than it is.

**Schema — new migration, will likely hit the same auto-mode classifier block as every other new-table prompt this project (084/355/085) — expected, route back through LIVE_STATE like Prompt 362 if so:**
1. New table `commission_schedule`: `carrier` (text), `type` (text, nullable — a few rows in the source data have no type), `product` (text), `tier` (integer), `pct` (numeric, nullable — null means no rate exists at that tier, e.g. F&G ExecuDex 61-99 and the 3 placeholder carriers below). Normalized long format (one row per carrier+product+tier), NOT 20 wide columns — makes the actual lookup query (`WHERE carrier = X AND product = Y AND tier = 70`) trivial.
2. Seed every row from `brain/commission-schedule-tier70.csv` — parse its wide format (one CSV row per product, 20 tier columns) into the normalized table on migration. **3 carriers are real but have zero rate data yet** (Baltimore Life, Aflac, Chubb — marked in the CSV as "AWAITING REAL DATA"): seed them as a single placeholder row each with `product = 'AWAITING REAL DATA...'` and all 20 tier rows `pct = NULL`, so the Compensation Grid page can still show these 3 carriers in the carrier filter (with dashes throughout, same convention as F&G ExecuDex 61-99's existing blank cells) rather than omitting them entirely. Brayden will get real numbers from someone with direct logins to those 3 carriers later — this is a placeholder, not a guess.
3. **Do NOT seed Augustar or SBLI** — Brayden confirmed neither is a real carrier for Ohvara (SBLI explicitly: "toss them, not on the overall portal"), even though both were present in the raw Eterna export. Drop them entirely, don't carry them into the schema at all.
4. Ohvara's own contracted tier (70) should be stored somewhere queryable too — a simple constant is fine for now (a single row in `app_settings`-style config, or a hardcoded constant in code with a comment explaining it's Brayden's real Eterna contract level as of 2026-07-26) rather than over-engineering a settings UI nobody asked for.

**Page — new "Compensation Grid" under Growth (near Commissions), mirroring Eterna's own layout (structure/interaction, not exact visual style — same rule as every past mockup-reference prompt):**
1. Carrier filter dropdown ("All carriers" + each real carrier) and Product Type filter dropdown ("All types" + distinct `type` values), same two-dropdown pattern as Eterna's page.
2. Table: Carrier / Type / Product columns, then one column per tier (50 through 145, step 5 — 20 columns) showing `pct`, rendering `—` for null (matches the app's existing missing-data convention, e.g. Carrier Portals' "No portal on file").
3. **Highlight the 70 column** (Ohvara's real contracted tier) the same way Eterna bolds/shades its own current-tier column — this is the whole point of the page, don't bury it as just another column.
4. Sortable columns optional/nice-to-have (Eterna's own page has sort arrows) — not required if it adds meaningfully more scope, use judgment.
5. Baltimore Life/Aflac/Chubb should appear in the carrier filter and render a single dashed-out row each — don't hide them just because they're empty, the whole point is Brayden can see at a glance which carriers still need real data.

**Note for later, don't build yet:** this table is also what the future real Commissions page will join against once policies capture their exact carrier+product (not just a broad category) — flagged in earlier conversation, not part of this prompt's scope, just don't design the schema in a way that makes that join awkward later (the normalized long format above should already be fine for this).

**Verify with a real screenshot** — page loads under Growth, both filters work, the 70 column is visually distinct from the other 19, F&G ExecuDex (61-99) and the 3 placeholder carriers all show dashes rather than blank/missing cells or errors, and Augustar/SBLI appear nowhere in the carrier filter.
