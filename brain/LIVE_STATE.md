...
### 🔲 Prompt 309 — Setter dashboard finalization: script rework, AI roleplay practice, video cards fix, add-to-home-screen prompt

Goal: get the setter (rep) desktop dashboard fully done before starting closer/admin UIs — same format/system gets piggybacked for those next. Four sub-items, queued together:

**(a) Script rework — non-presumptive, pain-discovery based.** The current live opener ("How many calls do you think you're missing while your guys are out on jobs?") presupposes the answer instead of discovering it. Rewrite `DISCOVERY_SCRIPT` (or whatever the live setter script source is — same one driving Call Modal / ScriptWalk) to open with a broad, non-leading gate question about call-handling day-to-day, not an assertion. Should cover multiple possible Voice-AI pain angles, not just missed calls — e.g. scheduling chaos, slow response, cost of hiring, current receptionist reliability — gate-then-drill pattern: only ask deeper quantifying follow-ups (how many missed/day, avg ticket, etc.) once a pain is actually confirmed. **Setter's job stops at ONE confirmed pain** — script culminates in transitioning straight to booking the meeting ("we found you because we saw you had this issue, we'll get someone to assess it and give you a quote"), not continuing into a deeper survey. The full quantifying/diagnostic survey now happens live on the CLOSER's call, not the setter's — that's future work, out of scope for this prompt (don't build closer-side survey logic here, just make sure the setter script cleanly hands off to booking).

**(b) AI roleplay practice mode around the reworked script.** Setters need a way to rehearse the new pain-discovery flow before going live. Check whether this can extend the existing AI Roleplay tab / Retell roleplay infra (already built per the 2026-06-12 5-part Manager batch — `RETELL_COACH_AGENT_ID`, TrainingCenter.jsx's AI Roleplay tab) rather than building new roleplay infra from scratch. AI plays a simulated prospect running through the reworked script.

**(c) Fix "video cards" — Brayden gave a corrected diagnosis 2026-07-18, changes the hunt.** This is very likely the SAME issue as [[Prompt 305 Black Bar Investigation]] (Training Center Videos tab, not reproduced across 6 widths in an unauthenticated empty-state Playwright harness). Prompt 305 searched for a COVERING element (hardcoded black overlay/bar rendering on top of the thumbnail) and found none — grepped for hardcoded black/`#000`/near-black hex, found only an unrelated `ErrorToast` color. **Brayden's correction: there is no covering element. The thumbnail image itself is shifted/moved down within its card, and the "black bar" is just the card's own background showing through the empty space left above it.** Reframe the investigation accordingly — look at the video thumbnail `<img>` (or background-image div)'s positioning within its container: `object-position`, `object-fit`, a `margin-top`/`top` offset, or a flex/grid alignment rule pushing the image down from where the container expects it to sit. Fix is likely just resetting that offset so the image sits flush at the top of its card again. Prompt 305's empty-state harness may not have reproduced this because the thumbnail shift could be tied to real loaded image dimensions/aspect ratio rather than the harness's placeholder-gray loading state — worth testing with a real thumbnail loaded, not just the 0ms placeholder frame. (Possibly related to the still-BLOCKED Task 2/Task 4 items from the 2026-06-13 Manager batch — video duration sourcing, quiz-to-video mapping — check those first before assuming this is new, but likely unrelated given Brayden's specific description.)

**(d) Settings tweaks — unscoped.** Brayden said "maybe a little tweaks in the settings" with no specifics. Flag back for exact scope before touching Settings — don't guess here either.

**(e) Add-to-home-screen / "Add Mobile App" install prompt — wants this now, not deferred.** Add a custom install-prompt banner/button in the setter dashboard. On Android/Chrome: intercept the native `beforeinstallprompt` event and trigger the real install flow from a button reading something like "Add Mobile App" (more intentional than a generic bookmark icon). On iOS Safari: no programmatic install API exists (Apple blocks one-click triggering) — show a custom instructional banner instead ("Tap Share, then Add to Home Screen"). Detect platform to show the right variant. This is for Brayden's own review pass through the dashboard first — confirm placement/copy with him if the obvious spot isn't clear, but the feature itself is approved to build now.

**Sequencing note (context, not a build instruction):** this is Stage 1 only — receptionist/Voice-AI scope, matching the locked North Star model and the Indeed lead source. The broader 5-channel (Voice/SMS/Web/Email/Social) survey system discussed this session is future-phase and NOT part of this prompt — see [[Memories]] 2026-07-18 session log for that full discussion if it resurfaces.
