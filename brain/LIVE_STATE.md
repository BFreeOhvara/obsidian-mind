...
### 🔲 Prompt 286 — "Mobile App" install box + QR code (investigate PWA feasibility first, report back before building)

**Context:** Brayden wants a way for reps to install the dashboard as a mobile app. Referenced this desktop Claude app's own UI as the visual/positional model: a small bordered box sits just above the account/settings footer in the sidebar (in Claude's case it says "Relaunch to update"); he wants an equivalent box in the same relative position in Ohvara's sidebar — above the account-name/settings/sign-out row at the bottom — labeled something like **"Mobile App"**. Clicking it opens a centered modal titled something like **"Scan QR Code"** showing a QR code that, when scanned on a phone, lets the rep add the dashboard to their home screen as an app. **If the rep is already ON mobile** (can't scan a QR code shown on the same device), the same box/modal should instead offer a direct install action — some equivalent of "Add to Home Screen" / "Make this an app" — rather than a QR code that's useless in that context.

**Investigate first — this depends on real PWA infrastructure that may not exist yet, don't assume it does:**
1. Does the dashboard currently have a web app manifest (`manifest.json`), service worker, and app icons configured at all? Check `index.html`/`vite.config` for any existing PWA plugin (e.g. `vite-plugin-pwa`) or manual manifest link. If none of this exists, "installable as an app" isn't real yet regardless of a QR code — report what's missing and what's needed to make the app genuinely installable (not just report a link to the same non-installable web page).
2. **Platform reality check, report honestly:** Android/Chrome supports a real programmatic install prompt (`beforeinstallprompt` event) — a "tap to install" button can work there. **iOS Safari does NOT support this API at all** — there is no programmatic "install" trigger on iPhone; the only path is the user manually tapping Share → Add to Home Screen themselves. If a meaningful share of reps are on iPhone, the "on-mobile, tap to install" half of this request may not be fully achievable as described — report this clearly rather than silently building something that only half-works, and propose the honest fallback (e.g. on iOS, show instructions for the manual Share → Add to Home Screen steps instead of a fake "install" button).
3. QR code generation: confirm whether a QR library is already a dependency anywhere in the repo, or if one needs adding (lightweight, e.g. `qrcode` npm package — small, no backend needed, generates client-side from the current URL).
