...
9. **CHAT UPDATE relay (NEW, 2026-06-12):** every CC prompt artifact generated by a Manager chat (web-Claude) includes a "CHAT UPDATE" section at the bottom summarizing what was discussed, decided, or figured out in the Manager conversation since the last CC prompt — separate from the task itself. When CC receives a prompt containing a CHAT UPDATE section, it folds that content into THIS FILE (Problems & Resolutions and/or Open Threads as appropriate) alongside its own task-completion log entry — same commit, same "done" step.
