...
**Step 1 — investigate first, don't guess at the date math.** Find `fillTokens()` in `discoveryScript.js` and confirm exactly which tokens are currently dynamic vs. literal (cross-check against the header comment's own token list — Eagle is going off that comment, not a full second read of every line, so confirm it's accurate/current). Report back the exact list of "in-call" tokens that still render literally today: at minimum `[Tuesday morning]`, `[Wednesday afternoon]`, `[Tuesday next week]`, `[Wednesday next week]` are in scope for this prompt — report whether `[day]`, `[time]`, `[time+1hr]`, `[owner]`, `[Name]` should also be considered (Eagle's read: those represent info captured FROM the prospect's own answer, not a forward-looking date offer, so likely stay literal — but confirm rather than assume, and flag if any of them turn out to need the same fix).

**Step 2 — propose the date-computation rule before building, since there's real ambiguity to resolve:**
- What should `[Tuesday morning]` / `[Wednesday afternoon]` resolve to if today already IS Tuesday or Wednesday — offer today, or skip to next week's occurrence? Propose a rule (e.g., "if today is Tuesday before some cutoff time, still offer today; otherwise the next upcoming Tuesday" — or simpler, "always the next occurring Tuesday/Wednesday, never today, to avoid a same-day ask reading as pushy") and report the reasoning before committing to code.
- `[Tuesday next week]` / `[Wednesday next week]` are explicitly used only in the "I don't have time this week" branch — these should unambiguously mean next calendar week's Tuesday/Wednesday, not "the next occurring one" (which could still be this week depending on today's day). Confirm the distinction is being preserved, not collapsed into one shared computation.
- Format: should the filled text read like "this Tuesday" / "Tuesday, July 14th" / "next Tuesday (the 14th)"? Propose one, consistent with how the other dynamic tokens currently render (plain business-appropriate text, no over-formatting).

**Step 3 — once the rule is confirmed (self-confirm and proceed if the reasoning is straightforward and matches this prompt's own stated assumptions, same pattern as Prompt 248 — only pause and report back if something genuinely ambiguous comes up that this prompt didn't anticipate), implement:** add the real date computation to `fillTokens()` (or wherever makes sense given what Step 1 finds), using the same timezone-aware pattern already established elsewhere in the codebase for "today" (`profiles.timezone`, per Prompt 244 — a rep in a different timezone than UTC should get a date computed from THEIR local day, not server UTC, consistent with the calendar fix already shipped this session).

**Verification:** `npx vite build` clean, then live-check in Training Center → Script practice AND (if practical) a real Call Now walk against an actual lead, confirming all 4 tokens render real, correctly-computed day references instead of literal brackets, across the Handoff pitch, "Just send me some info," "Who is this," "How much does this cost," and "I don't have time this week" branches (everywhere these tokens appear — grep to confirm the full list of usage sites before considering this done).
