...
**Update 2026-07-27 — Claude Chrome attempt came back empty, real access gap found.** Brayden ran a Claude Chrome session against `app.insurancetoolkits.com` looking for the embed/integration settings. Findings: the active session drops straight into the FEX Toolkit Lite quoter itself (`/fex/lite`) with no login prompt, but every other link (Account, CRM, the FEX product-switcher dropdown, any `/dashboard` sub-route) redirects straight to `insurancetoolkits.com/logout` instead of a real page — consistent across repeated attempts, not a fluke. The quoter page itself has no visible settings/gear/menu of its own. Claude Chrome correctly declined to dig into the app's minified JS bundle for a hardcoded token when its own tooling flagged that content as containing sensitive key/cookie-like data — right call, not something to work around.

**Working theory:** the session Brayden has live only has access to the bare quoter tool, not a real account dashboard — the embed/integration settings likely live behind a **separate login at the root `insurancetoolkits.com` domain** (distinct from `app.insurancetoolkits.com`), which Claude Chrome can't get into on its own since it can't enter credentials. **Needs Brayden to either:** (a) log into that separate root-domain portal himself if he has an account there and hand off to Claude Chrome from wherever it lands, or (b) just contact InsuranceToolkits support directly and ask for "the embed URL/token for the FEX Toolkit Lite quoter" — likely the faster path at this point given the self-serve UI hunt came up empty.

**RESOLVED 2026-07-27 — Brayden got the real token.** Real embed URL: `https://app.insurancetoolkits.com/fex/lite?token=f_6-6-vkNY_KFr8ZJN29BqsRFO-GVu-8QOblyla9` — same domain+path the code already uses, just missing the `?token=` param (simpler fix than the `lite-form` URL guessed earlier from web search; that guess was apparently a different/older embed pattern, this is the real current one for this account).

**Action for CC — handle this as a real secret, not just a URL string:**
1. Add it to `.env.local` as a new var (e.g. `VITE_INSURANCETOOLKITS_EMBED_TOKEN=f_6-6-vkNY_KFr8ZJN29BqsRFO-GVu-8QOblyla9`) — confirm `.env.local` is actually gitignored before writing to it, don't assume.
2. Update `Quoter.jsx` to build the iframe `src` from the base URL + `import.meta.env.VITE_INSURANCETOOLKITS_EMBED_TOKEN` — never hardcode the literal token in the component.
3. **Do not repeat the raw token value in any commit message, code comment, or future LIVE_STATE/Memories entry.** This entry is the one place it had to land since LIVE_STATE is the only Falcon↔CC channel — once consumed, edit this entry to replace the token value with `[REDACTED — moved to .env.local, see commit hash]` so it doesn't sit in this file's live content going forward (git history will still have it from this commit, which is an accepted tradeoff of using this channel — but the file's current content shouldn't keep displaying it).
4. Verify live with a real quote for a test client (real name, DOB, coverage amount) — confirm "Invalid Token" is gone and results actually populate the table underneath.
