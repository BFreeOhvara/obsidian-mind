...
### 🟨 Prompt 339 PARTIALLY SHIPPED 2026-07-25 — 2 of 4 done, 1 blocked (needs Brayden), 1 stopped before executing (needs Brayden decision)

**Shipped:** [`4b34332`](https://github.com/BFreeOhvara/ohvara-dashboard/commit/4b34332) on `ohvara-dashboard`.
...
**Item 1 (widen sidebar to match Eterna) — blocked, not done.** Tried to reach `portal.goeterna.com` to measure the real rail width as instructed ("rather than guessing a number") — it redirects to `auth.goeterna.com`, a login wall. No Eterna credentials are on file anywhere in the vault, and no reference screenshots from the earlier comparison exist in `media/` either (checked). Rather than guess a width against an explicit instruction not to guess, this is stopped here. **Needs from Brayden:** either (a) the actual Eterna rail width in px, (b) a screenshot of the Eterna sidebar with browser dev-tools computed width visible, or (c) Eterna login credentials so a session can measure it directly. Once one of those lands, this is a small, low-risk change (`Sidebar.jsx` line ~313, the `width: expanded ? 224 : 64` on the `<aside>`).
