...
### 🆕 Prompt 399 QUEUED 2026-07-31 — My Policies: align underwriting Yes/No banner with the effectuation banner + lock both to the submitting agent only

Two fixes to the "Submitted for underwriting — has [Carrier]'s decision come back?" banner ([[Prompt 369]]), using the existing effectuation banner ("Reached its effective date... did this policy go into effect?") as the reference pattern to match exactly:

**1. Visual alignment.** The underwriting banner's Yes/No buttons currently sit in a different position than the effectuation banner's. Make them match — same position, same layout, same spacing. Also add the same **"Not sure? Check [Carrier] portal ↗"** link the effectuation banner already has (linking to that policy's carrier, using whatever carrier-portal-URL source [[Prompt 358]]/Carrier Portals already uses) — the underwriting banner doesn't have this yet and should.

**2. Lock Yes/No to the submitting agent only — on BOTH banner types.** Right now these buttons are clickable by anyone who can see the row, which includes admin viewing "Everyone" on My Policies. Brayden's explicit call: **only the agent who actually submitted that policy (`agent_id = auth.uid()`) can click Yes/No on either banner** — not other agents, and not admin viewing someone else's policy in the "Everyone" scope. Everyone else should see the banner (for visibility/awareness) but with Yes/No disabled or hidden for them specifically.

Verify with screenshots: both banner types visually matching, and confirm via a second account (or admin's "Everyone" view) that Yes/No is inert on a policy that isn't yours.

---

### 🆕 Prompt 398 QUEUED 2026-07-31 — Exclude Test Agent (nate44) from every company-wide/"everyone" view — keep the account and its data intact

Now that real policies are flowing through the dashboard from actual agents, Brayden wants the Test Agent (`nate44`, `3f2b2df7-40b1-4921-80e2-09981c819642`) account **fully hidden from anywhere real company-wide data is shown** — but the account itself and its sample policies stay exactly as-is in the database, since Brayden still wants it available for his own testing. This is the same exclusion pattern already applied to Leaderboard/hierarchy rollups ([[Prompt 371]], migration 090's `recompute_leaderboard_rank()`) — extend it to the remaining surfaces that still show it:

1. **My Policies → "Everyone" toggle** (admin scope) — nate44's policies currently show up here; exclude them the same way Leaderboard already does.
2. **Team page → member/hierarchy list** — nate44 shouldn't appear as a team member anywhere on this page.
3. **Team → Messages** — nate44 shouldn't appear in the Conversations list (no DM option for it), and shouldn't be a member of "Team chat" (the company-wide channel) from any real user's point of view.

Do not delete anything, do not touch `nate44`'s own login/account — Brayden still needs to log into it himself to test features. This is purely about hiding it from everyone else's view of "the team"/"the company," consistent with how it's already excluded from Leaderboard.

Verify with screenshots: My Policies "Everyone" toggle with nate44's policies absent, Team page member list without Test Agent, and Team → Messages conversation list without Test Agent.
...
Reframes what the Meetings tab is — Brayden does NOT want a schedule-a-meeting calendar. He wants a persistent, always-open room agents can drop into anytime (camera on/off, mute, ask questions, "everyone's selling insurance in there" — like an open bullpen, not a booked call). Confirmed scope for v1: **just ONE room, prove it works, expand to multiple channels later only if this pans out.** Don't build multi-channel infrastructure yet.

**Asked Brayden directly 2026-07-30 (`AskUserQuestion`)** for Step 1's Zoom URL — he doesn't have one set up yet and chose to hold this prompt rather than build against a placeholder. [[Prompt 392]] shipped in the meantime and already gives this a real home to land in (`app_settings.zoom_room_url`, editable from Settings → Integrations, admin-only, singleton row confirmed `null` in the DB). Once Brayden has a link, either he pastes it into Settings → Integrations himself, or hands it to CC/Eagle/Falcon to wire in — either path works now that the schema exists. Resume from Step 1 below once the link exists.

**Step 1 — get the real room link.** A Zoom account's Personal Meeting Room is one permanent, non-expiring meeting link per account. Ask Brayden directly (`AskUserQuestion`, same as the [[Prompt 369]] design-call precedent) for his/Ohvara's Zoom Personal Meeting Room URL — or if there isn't one set up yet, ask him to grab it from zoom.us account settings, or create a "recurring meeting, no fixed time" if he wants the room separate from his personal one. This is real account setup outside the codebase, not something to build around blindly.

**Step 2 — try embedding it, but verify, don't assume.** Same lesson as Carrier Portals/Quoter blocking iframes ([[Prompt 372]]) — Zoom's web client may or may not allow being iframed for security reasons. Try it live. If it embeds cleanly inline on the Meetings tab, great. If it's blocked, fall back to a big "Join Live Room" button that opens the Zoom link in a new tab — don't fight a blocked iframe, same pattern as before.

**Step 3 — admin "notify team to join" button.** Ties directly into the real notification system from [[Prompt 384]]/migration 090 — add a manual (not automatic) admin-only button that fires a notification to every agent with a message like "Live Room is open — join now" and a link/deep-link into the room. New notification type, e.g. `live_room_invite` — manually triggered by an admin clicking a button, not tied to any automatic DB event like the other 4 real triggers.

Verify with a screenshot of the Meetings tab showing the Live Room (embedded or button-to-new-tab, whichever the real embed test determines) and the admin notify button.
...
5. **SCOPING NEEDED — agent-entered carrier commission info to auto-compute real commissions.** Raised by Brayden 2026-07-25 alongside Prompt 339. Idea: add fields in Settings/Profile where each agent enters their own carrier-specific commission info (comp rate/rating, whatever isn't public) so that when a submission is made, Commissions can compute the real payout automatically instead of showing the current "projected" placeholder. **Explicitly not scoped yet — Brayden said himself he doesn't know what fields or formula are needed.** Needs a real scoping pass before this becomes a buildable prompt: what per-carrier data point(s) actually drive the payout math, whether it's the same field(s) across all 12 carriers or carrier-specific, and what the "reverse engineer the payout" formula actually is. Don't guess a schema — ask Brayden directly next time this comes up.
