...
**Shipped:** [`4b34332`](https://github.com/BFreeOhvara/ohvara-dashboard/commit/4b34332) + [`8a6e1ba`](https://github.com/BFreeOhvara/ohvara-dashboard/commit/8a6e1ba) on `ohvara-dashboard`.
...
**Item 1 (widen sidebar to match Eterna) — done at 260px (estimate), exact measurement still available if wanted.** `Sidebar.jsx`'s `<aside>` widened from `expanded ? 224 : 64` to `expanded ? 260 : 64` (collapsed unchanged — Eterna has no collapsed state to compare against). 260 was Falcon's eyeballed estimate from Eterna screenshots, explicitly approved by Brayden in place of an exact measurement (Eterna is magic-link-only, no password to hand over). Kept `DashboardLayout.jsx`'s `--sb-w` CSS var and `index.css`'s `.app-main` margin-left fallback in sync with the new value so the main content column doesn't gap or overlap the wider rail. `eslint` clean on both touched files (pre-existing unrelated errors on `DashboardLayout.jsx`:97 and `Sidebar.jsx`'s `Calculator` import untouched, same ones flagged in 337/338). `vite build` clean. Live-verified: expanded `<aside>` computed width is exactly 260px, `.app-main`'s `margin-left` matches at 260px (no gap/overlap), and none of the nav labels overflow at the new width — checked every closer-role label including the two longest ("Carrier Portals", "Training Center") via `scrollWidth`/`clientWidth`, all clean. Didn't re-check admin-role labels ("Closer Roster", "Users & Access") — none is longer than what was already verified, low risk, but worth a glance if Brayden spots anything on that role.

**Note found after shipping:** a handoff doc landed mid-session (`brain/claude-chrome-measure-eterna-sidebar.md`, committed by Eagle as `b0a79cf`) pointing out Brayden's own Chrome already has a live, logged-in Eterna session open, so the *exact* px width is now gettable via Claude in Chrome rather than eyeballed — 3 short steps (inspect the rail, read computed width, report the number). Tried it this session — the Claude in Chrome extension wasn't connected here, so couldn't pull the real number. **260 stands as the shipped value.** If Brayden wants the exact figure, either he runs those 3 steps himself (they're written for a human, no dev skill needed) or a future CC session with Claude in Chrome connected can. If the real number differs meaningfully from 260, it's a one-line change in three places (`Sidebar.jsx`'s width, `DashboardLayout.jsx`'s `--sb-w`, `index.css`'s `.app-main` fallback) — not worth re-opening unless Brayden flags the current width looks off.
