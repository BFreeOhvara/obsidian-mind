...
### Prompt 320: Mobile PWA — persistent login + branded splash animation on launch — NOT YET BUILT

**Source: Brayden, Falcon manager-chat session 2026-07-20.** Two related mobile-only asks. Read the real code before touching anything — `src/lib/installPrompt.js`, `src/hooks/useInstallPrompt.js`, `src/lib/platform.js` (has `isStandalone`), `src/components/layout/MobileAppModal.jsx`, and whatever initializes the Supabase client (session persistence config) all already exist per Prompts 286/291/296/309(e) recon — this builds on top of that, not from scratch.

**Explicit non-goal:** desktop is NOT in scope. Brayden's fine with desktop's current behavior (browser tab/bookmark requires re-login unless the password's saved in a browser password manager) — don't touch desktop auth/session behavior, this prompt is mobile-installed-PWA only.

**1. Persistent login on the installed mobile app.** Currently, opening the app from the home-screen icon (standalone/PWA mode) requires logging in every time, even for a setter who's already authenticated. Investigate root cause first — likely candidates: the Supabase client's `persistSession` config, whether the PWA's storage (localStorage/IndexedDB) is actually being retained across launches in standalone mode (some install/manifest configs can affect this), or a session-check bug that's treating a valid persisted session as invalid on cold launch. Fix so a setter who's already logged in stays logged in when reopening the installed app — same expectation as any normal native app.

**2. Branded animated splash screen on cold launch, standalone/PWA mode only.** Brayden wants something in the spirit of Supercell's mobile games (Clash Royale/Clash of Clans) — a brief logo animation (fade/scale/pulse, a couple seconds) that plays before the actual app content appears, instead of a blank flash or an instant jump to the login/dashboard screen. Scope this to `isStandalone` (already available in `platform.js`) so it only fires when launched from the home-screen icon — not on every desktop or regular mobile-browser page load/refresh, which would be excessive. Exact animation style is a creative call — reasonable default: Ohvara logo fades/scales in, brief pulse or shimmer, then transitions into whatever screen the (now-persistent, per item 1) session lands on. Flag the specific animation choice back to Brayden as a quick visual check rather than assuming it's exactly right on the first pass.

**Verification:** live-test both on a real mobile device or an accurate standalone-mode simulation (headless preview can't fully replicate PWA install/launch behavior) — confirm a logged-in setter reopening the installed app lands straight on the dashboard, no login screen; confirm the splash plays once per cold launch, not on every internal navigation; confirm desktop and non-installed mobile-browser behavior are unchanged. `npx vite build` clean.
