...
### Prompt 195 — BUG: My Leads goes empty for ~6 hours every night, well before the on-screen countdown hits zero (root-caused by Falcon, not just reported)

**Context (Brayden, 2026-07-01):** reported leads "leave the box" around 7pm Central even though the "Leads refresh Xh Ym" countdown on `/rep` hadn't reached zero yet, and asked why. Falcon root-caused this directly against the live repo (GitHub code search) and the live Supabase DB (direct SQL) — this is a real, confirmed bug, not a misunderstanding of time zones.

**Root cause (confirmed, not speculative):**
- `src/hooks/useLeads.js` line 10: `const today = new Date().toISOString().split('T')[0]` — computes "today" as a **UTC** calendar date, then `useMyLeads()` filters leads with `.eq('batch_date', today)` (line 15).
- The live Supabase Postgres instance's timezone is UTC (confirmed via `SHOW timezone` / `now()`), and the live `assign_daily_batches()` function (confirmed via `pg_get_functiondef`) sets `batch_date = current_date` — also a UTC date.
- The live cron job `daily-batch-assign` (id 12, `SELECT assign_daily_batches()`) runs at schedule `5 6 * * *` = **06:05 UTC = 1:05 AM Central (CDT)** — confirmed via `cron.job`. This matches the countdown target already hardcoded/commented in `MyLeads.jsx` ("Batch reset countdown target — 06:05 UTC (1:05 AM Central)").
- **The gap:** at UTC midnight (7:00 PM Central during CDT), `today` in `useLeads.js` flips to the new UTC date immediately — but `leads.batch_date` for the rep's current batch doesn't advance to that new date until `assign_daily_batches()` actually runs, 6 hours 5 minutes later at 06:05 UTC. During that entire window, `.eq('batch_date', today)` matches **zero rows**, so the rep's screen goes empty — while the countdown (correctly counting down to the real 06:05 UTC reassignment) still shows hours remaining. Two different clocks: the query's day-cutover (UTC midnight) and the actual reassignment (06:05 UTC) are 6h5m apart, and nothing bridges that gap today.
- **This is a resurfacing of a previously-diagnosed bug**, not a new class of issue: migration `016_daily_batch_cron.sql`'s own header comment literally says *"Root cause being fixed: useMyLeads filters leads on batch_date = <UTC today>, but nothing advanced batch_date when the UTC day rolled over, so reps woke up to an empty dashboard."* Whatever that migration shipped either didn't fully close this window, was later bypassed, or the cron's live schedule has drifted since (see note below) — CC should read `016_daily_batch_cron.sql` in full before touching anything, since it already contains the prior fix attempt and reasoning.
- **Side note, not necessarily the cause but worth a look while in there:** there appear to be two separate live cron jobs both related to batch assignment — `assign-daily-batch` (id 1, `0 6 * * *`, calls an edge function via `net.http_post`) and `daily-batch-assign` (id 12, `5 6 * * *`, calls `assign_daily_batches()` directly). These may be an old/new pair where one is dead weight — worth confirming only one is actually supposed to be active and cleaning up the other if so, since two similarly-named jobs 5 minutes apart doing (possibly) the same thing is confusing and risks double-processing.

**Fix direction (Brayden's stated expectation: "they should only refresh when the timer is done"):** make the rep's lead list resilient to this gap so it never goes empty before the real reassignment happens — e.g. stop requiring an exact match against an independently-computed "today," and instead show the rep's most recent/current batch (the leads matching their latest `batch_date`) until a new batch actually supersedes it. Alternative: align the two cutover points so they happen at the same instant. Use your judgment on the cleanest implementation given what's already in migration 016 — the important thing is the dashboard should never render empty while the countdown still shows time remaining.

**Do NOT change:** the cron's actual 06:05 UTC schedule/countdown target unless the duplicate-cron investigation above shows it's genuinely wrong — that part of the system is working as intended per Brayden's 2026-06-22 reschedule; this fix is about the query-side gap, not the cron timing itself.

**Verify:** Confirm `useMyLeads` (or its underlying query) no longer returns empty for any rep between UTC midnight and 06:05 UTC when they still have an active batch from the prior day. If practical, simulate/inspect by checking a rep's `leads` rows around a `batch_date` boundary rather than waiting for real UTC midnight to test live.
