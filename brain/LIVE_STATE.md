...
### 🔲 Prompt 231 QUEUED 2026-07-06 (Eagle, from Brayden's live screenshot review) — Activity Feed divider match, calendar start-day star, Payouts scroll cap, Commissions range-filter + live stat boxes, My Calls average grade, LiveClock reorder/format/box

Six independent UI fixes from Brayden reviewing the live rep dashboard (screenshots: Activity Feed, My Calls, My Commissions, My Leads header clock). Ship in whatever commit grouping is efficient — throughput matters more than commit count. Verify ALL parts live via temp preview routes per the standing convention (mocked `AuthContext` + seeded react-query cache), remove temp routes before commit, `npx vite build` passes, `git status --short` clean.

**A. Activity Feed row divider — match My Calls exactly (`src/pages/rep/ActivityFeed.jsx`)**
Prompt 228 gave Activity Feed a divider between rows only (`isLast ? 'none' : '0.5px solid var(--border)'`). Brayden compared both pages side by side and wants Activity Feed to match My Calls' behavior: **unconditional** `border-bottom: 0.5px solid var(--border)` on every row including the last — same as `MyCalls.jsx`'s existing per-row divider (Prompt 228 item 3). Drop the `isLast` conditional.

**B. Calendar "start day" star — shared `DayFilterBar`/`useDayFilter` (`src/components/ui/DayFilterBar.jsx`), used by both Activity Feed and My Calls**
Add a star marker on whichever calendar day = the rep's first-ever call (`MIN(calls.created_at)` for that rep, bucketed to whatever day-boundary convention this component already uses — UTC calendar day per Prompt 223/225). Star should render on that day regardless of which month is being browsed (permanent marker, not limited to the current view) so the rep can always find "day one" quickly. One shared fix covers both pages since they already share this component.

**C. My Payouts box — cap to 5 visible rows, scroll internally for the rest (rep Commissions page — confirm actual filename, e.g. `src/pages/rep/Commissions.jsx`)**
The Payouts list currently shows more than 5 rows unscrolled. Reduce the container's visible height by ~2 rows so exactly 5 payout rows show before an internal scroll kicks in (`overflow-y: auto` scoped to the list container, not the page) — bounded-box-scroll pattern, not a page-level scroll.

**D. Commissions page — date-range calendar drives the 3 top stat boxes**
**Investigate + report first:** the exact 3 stat boxes currently at the top of this page (Brayden named "Total Earned" and "Deals Closed" — get the real 3rd), what computes them today, and whether there's a static "Last 7 Days" label that doesn't actually respond to any existing filter (Brayden's report). Then **build:** add a calendar control reusing the **range-select pattern already built for the closer dashboard** (`RevenueTracker.jsx`'s `MiniCalendar` — contiguous start+end click-to-select with hover preview) — explicitly NOT the single-day-only `DayFilterBar` pattern from part B; Brayden wants a real 2-endpoint range here, unlike Activity Feed/My Calls. Selecting a range recomputes all 3 top stat boxes live over that range. Remove/replace whatever static "Last 7 Days" label doesn't currently respond to filtering.

**E. My Calls — overall average-grade display (`src/pages/rep/MyCalls.jsx`)**
Add an overall average-grade indicator outside/above the per-day call list (placement similar to Activity Feed's calendar trigger) — an **all-time average across every graded call this rep has ever had**, NOT scoped to the currently-selected day. **Investigate first** how grades are actually stored (raw numeric score with a letter derived for display vs. letter-only) before picking the averaging method — report back. If a numeric score exists, average that and map back to the nearest letter; if only letters are stored, use a standard letter→numeric-scale mapping to average and round back to a letter, and flag that as an assumption in the log.

**F. LiveClock — reorder, 12-hour+AM/PM, drop TZ abbreviation, box around the time only (`src/components/ui/LiveClock.jsx`)**
Prompt 229 made this 24h/no-AM-PM/no-box/no-seconds. Brayden reviewed it live and wants: (1) **date before time** (currently time-then-date) — e.g. "Monday, July 6" first, then the time; (2) **switch to 12-hour clock WITH AM/PM** (e.g. "12:11 AM") — confirmed explicitly by Brayden over 24-hour when asked directly, reversing part of Prompt 229's format choice; (3) **drop the timezone abbreviation** ("CT" etc.) entirely — the clock always shows the viewer's own local time by construction, no label needed; (4) **a bordered box specifically around the time portion only** (not the date text) — simple border/pill, per Brayden's "easier to see" ask. Keep the live 1s-tick + per-viewer-`profile.timezone` behavior from Prompts 227/229 unchanged — only reorder/reformat/restyle.
...
6. **2026-06-12 — PS 5.1 REST verification traps (two).** (a) `@(Invoke-RestMethod).Count` returns 1 for an empty PostgREST result — PS 5.1 hands back the literal string `"[]"`, which array-wraps to one element; count via raw `.Content` or the Content-Range header i