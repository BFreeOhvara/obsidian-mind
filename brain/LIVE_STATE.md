...
### 🔲 Prompt 310 — Call-outcome integrity: Calls Today only counts real outcomes, block status changes before a call is made

**(a) "Calls Today" should only increment on a real call outcome — voicemail reached or actually talked to the person — not on every dial attempt or unrelated status change.** Investigate current logic (`calls` table insert path, `useTodayCallStats` in `src/hooks/useProfiles.js`, and however CallModal currently writes a call row) to determine exactly what triggers the count today, then correct it so ONLY those two outcomes (voicemail / answered) increment it. Report back what the current trigger actually was before assuming — don't guess at the fix without confirming the current behavior first.

**(b) Block setting a lead's status before a call has actually been made, with an error toast.** If a rep tries to change a lead's status without having clicked "Call Now" for that lead first, block it and surface an error using the EXISTING top-right slide-in `ErrorToast` component (already in the codebase — `position:fixed`, `#13131F` background, seen referenced in [[Prompt 305 Black Bar Investigation]]) rather than building a new toast. Copy along the lines of "You have to make the call first" (exact wording Brayden's call — propose a default, don't block on it). Investigate how (or whether) "has this lead been called yet" is currently tracked before building the guard — may need a new piece of state per lead/attempt if nothing tracks this today.

**Context only, not part of this prompt:** Brayden asked whether call recording starts before or after the client picks up. Per existing documentation (Open Thread 4, "Call recording — reframed," further up this file), there is currently NO real call recording — "Call Now" is a bare `tel:` link opening the rep's own phone's native dialer, no audio ever reaches the dashboard, the Twilio Voice browser-dialer scaffold is dormant with no edge function behind it. Already flagged as its own future initiative (real telephony layer + multi-state consent-disclosure design, since leads span TX/NC/CO's mixed one-party/two-party consent rules) — no action here unless Brayden explicitly wants to kick that off.
