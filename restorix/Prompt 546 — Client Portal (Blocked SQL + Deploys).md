...
## Step 2 — Edge-function redeploys (2 functions, both blocked for CC this session)

Source is committed in the repo working tree at:
...
## Step 3 — CC ships the frontend

Once 1a+1b are committed and Step 2 is deployed, CC (next session) runs `npm run build` (already clean locally), commits the staged working tree, pushes to `main` on `restorix-setter-portal`, confirms the Vercel bundle hash matches, and runs the verification loop below.

### Staged frontend files (all uncommitted in the working tree right now)
| File | Change |
|---|---|
...
## Step 4 — Verification loop (full, on real production, per the prompt)

As `test_closer` on `portal.restorix.co`:
