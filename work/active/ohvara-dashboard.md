...
## Prompt 4 — Custom Stack + Closer Demo Dashboard (shipped 2026-06-20)

À la carte, formula-priced recommendation replacing the fixed 4-tier packages for the closer flow. `recommend-stack` formula: `callsMissedPerWeek × 4.33 × avgTicket × 0.15` → monthly price (floor $297, ceiling $1,797, round to $10), drawn from an 8-automation catalog. `CallModal.jsx` fires it on `Appointment Booked` and caches the full result on the lead row (`recommended_automations`/`custom_monthly_price`/`recommended_stack`/`stack_generated_at`, migration 034); `AppointmentCard.jsx` loads the cache first, falling back to a live call only if uncached. New `SampleDashboard` component gives Nate a collapsible "Preview for [Business Name]" with per-automation tabs during the close call.

**Two RLS bugs found + fixed during visual verify** (appointments existed but closers couldn't see them): migration `035_closer_unassigned_appointments.sql` — `appointments_closer_select`/`_update` only allowed `closer_id = auth.uid()`, so a pending appointment stayed invisible to every closer until someone explicitly assigned it (which only ever happens at close, not at booking) — broadened to also allow any `role='closer'` to see/update unassigned (`closer_id IS NULL`) rows. Migration `036_closer_leads_via_appointment.sql` — `leads_rep_select` only granted closer access via `assigned_closer_id` (set by a separate `assign-closer` edge function, never automatically) — broadened to let any closer read a lead that has an appointment row at all. Both verified live via direct Supabase API calls as `nate44`. Full build/fix trail: [[Memories]] 2026-06-19 + 2026-06-20 entries.

## Prompts 7+8 — Real Demo Client at Booking + Close/Lost Lifecycle (shipped 2026-06-20)

Booking an appointment now auto-provisions a REAL client account in demo mode (`provision-demo-client`, chained after `recommend-stack`'s cache-write to avoid reading stale data) — Nate gets actual login credentials to open the prospect's dashboard live on the close call, not just an in-card preview. Migration 038: `clients.status` gains `demo`/`lost`, `clients.lead_id` + `recommended_automations` snapshot, `appointments.demo_client_id` + `demo_credentials`. Close converts the SAME clients row in place (extended `provision-client`, reuses the login, swaps the `demo-{leadId}@ohvara.internal` email for a real one if Nate provides one); lost deletes the demo account entirely (new `cleanup-lost-demo` fn, wired into the existing outcome='lost' Save Outcome flow rather than a new button).

**Found and fixed because the demo would have been pointless without it:** `/client/*` portal pages (`ClientOverview.jsx`/`ClientAutomations.jsx`) were still showing a fixed Basic/Pro/Premium/Elite catalog with tier-upgrade UI — stale since Prompt 5. Rewrote both to show the real AI-generated automations + a Demo Preview banner; extracted the synthetic-stats generator to `src/lib/syntheticStats.js` so the client portal and Nate's SampleDashboard show identical numbers.

**Real billing bug found mid-verification:** `provision-client`'s billed `monthly_value` always fell back to the old fixed `getTierPrice(tier)` unless Nate typed an override — silently ignoring the actual custom formula price entirely. Every close was charging the rounded tier price, not the quoted price. Fixed (override_price > recommendedPrice > getTierPrice fallback) and re-verified.

Full end-to-end verified via two booking-to-outcome cycles (real lead → demo → close-converts-in-place / real lead → demo → lost-deletes-cleanly) via direct API calls. Full trail: [[Memories]] 2026-06-20.

## Prompts 5+6 — Problem-First Custom Stack, No Fixed Packages (shipped 2026-06-20)

Replaces Prompt 4's catalog-and-tier model entirely: no more Basic/Pro/Premium/Elite anywhere, no fixed automation catalog. `CallModal.jsx` now captures structured discovery (calls missed/wk, avg ticket, primary pain, current setup, secondary pain) when booking — this was a genuine blocking gap, not a refinement: those columns existed since migration 034 but nothing wrote them, so formula pricing had never actually fired in prod before this. `recommend-stack` generates 2-5 free-form `{name, description}` automations per lead (no catalog constraint) from that discovery data. Setup fee $497→$297 everywhere in code (CLAUDE.md, North Star, Commissions.jsx, provision-client, Overview.jsx) — **except the real Stripe Payment Links, still $497 in Stripe itself, needs Brayden to create new $297 links**. `AppointmentCard.jsx`'s old "Alternative Packages" UI (fixed-tier cards with their own Stripe links) was removed — one custom stack, one price now; `SampleDashboard` synthesizes per-automation sample stats deterministically instead of a fixed lookup table. Migration 037 (`leads.primary_pain`/`current_setup`, `clients.setup_fee` default). Full end-to-end flow (setter discovery → recommend-stack → closer view) functionally verified via direct API calls mirroring the real auth/booking flow — no actual screenshot walk has happened yet (Chrome MCP unreachable all session). Full trail: [[Memories]] 2026-06-20.
